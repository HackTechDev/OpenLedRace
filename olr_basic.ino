...
void setup() {
  ...
  if ((digitalRead(PIN_P1) == 0)) {     ENABLE_RAMP = 1;
...
void winner_fx(byte w) {
  ...
  for (int note = 0; note < msize; note++) {
    if (SMOTOR == 1) {
      tone(PIN_AUDIO, win_music[note] / (3 - w), 150);
    } else {
...
void loop() {
  ...
  //move player 1 & 2 according to respective speed
  dist1 += speed1;
  dist2 += speed2;

  //sound for new leader
  if (dist1 > dist2) {
    if (leader == 2) {
      FBEEP = 440;
      TBEEP = 10;
  ...
  // note : didn't understood... :(
  // maybe change drawing order each 512ms ??
  ...
  //draworder = 1 if player 1 goes faster
  //          = 0 if player 2 goes faster
